...
function hashSmiles(smiles: string): string {
	return createHash("sha256").update(smiles).digest("hex");
}
...
export async function GET(req: Request) {
	try {
		...
		const normalizedQuery = normalizeLinkerInput(query);
		const smilesLike = looksLikeSmiles(query);

		let matched: LinkerResolveRow | null = null;
		...
		if (!matched) {
			matched = await findByAlias(normalizedQuery);
		}
		...
		const body: LinkerResolveResponse = {
			query,
			normalizedQuery,
			inputMode: smilesLike ? "smiles" : "alias",
			matched: Boolean(matched),
			canonicalSmilesHash: matched?.canonical_smiles_hash ?? canonicalSmilesHashFromInput,
			canonicalSmiles: matched?.canonical_smiles ?? canonicalSmilesFromInput,
			displayName: matched?.display_name ?? null,
			aliases,
			suggestions: matched ? [] : await findSuggestions(normalizedQuery),
		};
